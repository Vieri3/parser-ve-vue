...
export function useRoutes(fastify, options, done) {
    // с фронтенда приходит запрос к fastify с адресом "RES_GET_ARCHIVES" (данные записаны в constants.js)
    ...
    // после выполняется асинхронно функция parsePageArchives(); парсинга которая находится в handles.js
    // функция parsePageArchives(); принимает параметр который мы передали и делает запрос на сайт по ссылке которую мы передали в теле
